...
function loadGrid() {
    ...
    $('#dataGrid').datagrid({
        ...
        columns: [[
                ...
                {field: 'group_name', title: 'Group'},
                {field: 'isActive', title: 'Active'},
                {field: 'OpenningDr', title: 'Opening Dr'},
                {field: 'OpenningCr', title: 'Opening Cr'}
            ]]
...
function Edit() {
    ...
    if (row) {
        ...
        $('#fm').form('load', row);
        url = 'ledger_save.php?mode=""&search_id=' + row.LedgerID;
    }
}
function Save() {
    //alert(22);
...
function Remove() {
    ...
    if (row) {
        $.messager.confirm('Confirm', 'Are you sure you want to remove this user?', function(r) {
            if (r) {
                $.post('ledger_save.php?mode=delete',
                        {
                            search_id: row.LedgerID
                        },
                function(result) {
                    if (result.success) {
                        $('#dataGrid').datagrid('reload'); // reload the user data
