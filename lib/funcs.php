...
function allMarketingMaterial() {
    ...
    while ($stmt->fetch()) {
        $row[] = array('marketing_materials_id' => $marketing_materials_id,
            ...
            'balloon_qty_id' => $shirt_tqy_name, 'balloons_button_qty_name' => $balloons_button_qty_name,
...
function deleteMarketingMaterialRows($leadsMaterial) {
    //print_r($leadsMaterial); die();
    global $mysqli, $db_table_prefix;

    foreach ($leadsMaterial as $key => $value) {
        $stmt = $mysqli->prepare("DELETE FROM " . $db_table_prefix . "marketing_materials 
		WHERE marketing_materials_id = '$value'");
        $stmt->execute();
    }
...
function fetchInActiveFranchise() {
    global $mysqli, $db_table_prefix;
    $stmt = $mysqli->prepare("SELECT 
		id,
		fname,
		...
		active,
		date_modified
		FROM " . $db_table_prefix . "franchise WHERE active=0");
    $stmt->execute();
    $stmt->bind_result(
            $id, $fname, $lname, $ssefin, $city_state_zip, $phone, $email, $date, $active, $date_modified);
...
function fetchActiveFranchise() {
    ...
    $stmt = $mysqli->prepare("SELECT 
		...
		active,
		date_modified
		FROM " . $db_table_prefix . "franchise WHERE active=1");
    $stmt->execute();
    $stmt->bind_result(
            $id, $fname, $lname, $ssefin, $city_state_zip, $phone, $email, $date, $active, $date_modified);
