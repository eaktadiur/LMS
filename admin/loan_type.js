...
function AddNew() {
    ...
    $('#fm').form('clear');
    url = 'loan_type_save.php?mode=new';
}
function Edit() {
    ...
    if (row) {
        ...
        $('#fm').form('load', row);
        url = 'loan_type_save.php?mode=""&search_id=' + row.LoanTypeId;
    }
}
function Save() {
    //alert(22);
...
function Remove() {
    ...
    if (row) {
        $.messager.confirm('Confirm', 'Are you sure you want to remove this user?', function(r) {
            if (r) {
                $.post('loan_type_save.php?mode=delete',
                        {
                            search_id: row.LoanTypeId
                        },
                function(result) {
                    if (result.success) {
                        $('#dataGrid').datagrid('reload'); // reload the user data
