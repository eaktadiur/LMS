...
class ExportarRelXML {
	static function RelatorioPrinc($id,$addwhere,$from_rel,$from_form,$base,$template,$template_tipo,$template_nume,$sqlselect,$sqlfrom,$sqlwhere,$sqlorder,$sqlgroup,$sqlformgrup,$conecta,$conectaRel){
		$query = "SELECT CAMCAMPO FROM PHPREP_CAMPO WHERE RELCODIGO=".$id." ORDER BY CAMORDEM";
				...
				if ($linhas > 0) {
					...
					while($j = $result->fetchRow(MDB2_FETCHMODE_OBJECT)) {		
						...
						for($id_coluna=0;$id_coluna<$colunas;$id_coluna++){
							...
							if (MDB2::isError($result1)) die ($result1->getMessage()); 
							$i = $result1->fetchRow(MDB2_FETCHMODE_OBJECT);
							$titulo = $i->CAMCAMPO;
							if ($dado!=null) print "<$titulo>".$dado."</$titulo>";
		...
		if ($rows > 0){
			...
			if ($rows2 > 0){
				...
				for($cont=0;$cont<count($formulas);$cont++) {
					...
					print "<$pegatitulo>".$formula."</$pegatitulo>";
				}
			}
		} 
	}
		...
		if ($rows > 0){
			while($i = $result->fetchRow(MDB2_FETCHMODE_OBJECT)){
				...
				if($grupo==$tam-1) {
					...
					ExportarRelXML::RelatorioPrinc($id,$addwhere,$from_rel,$from_form,$base,$template,$template_tipo,$template_nume,$sqlselect,$sqlfrom,$sqlwhere,$sqlorder,$sqlgroup,$sqlformgrup,$conecta,$conectaRel);
				} else if ($grupo<$tam) {
					ExportarRelXML::RelatorioGrupo($id,$grupo+1,$agrupar_por,$tam,$where,$from_rel,$from_form,$base,$template,$template_tipo,$template_nume,$sqlselect,$sqlfrom,$sqlwhere,$sqlorder,$sqlgroup,$sqlformgrup,$conecta,$conectaRel);
				}
	...
	static function VisualizarSalvo($id)
	{
		...
		if ($grupos > 0){
			...
			ExportarRelXML::RelatorioGrupo($id,0,$agrupar_por,count($agrupar_por),null,$from_rel,$from_form,$base,$template,$template_tipo,$template_nume,$sqlselect,$sqlfrom,$sqlwhere,$sqlorder,$sqlgroup,$sqlformgrup,$conecta,$conectaRel);
		} else ExportarRelXML::RelatorioPrinc($id,null,$from_rel,$from_form,$base,$template,$template_tipo,$template_nume,$sqlselect,$sqlfrom,$sqlwhere,$sqlorder,$sqlgroup,$sqlformgrup,$conecta,$conectaRel);
