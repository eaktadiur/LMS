 ...
 * repgen-select.php for PHP Report Generator 
   Bauer, 22.1.2002 
   Version 0.2 
*/ 
...
while ( is_array($_REQUEST) 
  ...
  case "change": 
	    ...
	    switch (substr($id,0,1)) {  // Change a Block 
              case 'B': 
                $url=REPGENDIR."/repgen-createblock.php?".SID; 
              ...
              case 'F':   // Change Function 
                $url=REPGENDIR."/repgen-createfunct.php?".SID; 
                $url.="&id_neu=$id&short=".urlencode(get_kurzn($attr))."&long=".urlencode(get_name($attr))."&author=".urlencode(get_author($attr)); 
                $url .= "&id=".$id; 
                break; 
...
//echo $HTTP_HOST.$url; exit; 
            header("Location: http://$HTTP_HOST".$url);  // switches to repgen-create?.php 
            exit; 
 
          break; 
  case "delete": 
                // deletes report(all records) with id from table reports 
            $url=REPGENDIR."/repgen-del.php?".SID; 
  ...
  case "copy": 
	    ...
	    if (substr($id,0,1) == 'F')$n = 'F'.$n; 
            ...
            for ($k=0;$k<$l;$k++) { 
                $query1="insert into reports values('$n','$typ_ar[$k]','$attrib_ar[$k]')"; 
  ...
  case "newblock": 
	    ...
	    $n = 'B'.$n; 
            ...
            header("Location: http://$HTTP_HOST".$url);  // switches to repgen-createblock.php 
            exit; 
  ...
  case "newfunct": 
	    ...
	    $n = 'F'.$n; 
            $url=REPGENDIR."/repgen-createfunct.php?".SID; 
            $url.="&id_neu=$n&short=&long=&author=&group=&sql=&print_format=&print_size=&report_type=&id=&group_type="; 
            header("Location: http://$HTTP_HOST".$url);  // switches to repgen-createfunct.php 
            exit; 
...
<form name="navigate" method="post" action="repgen-select.php?<?=SID ?>"> 
...
?> 
 <!-- existing reports --> 
 <form name="edit" method="post" action="repgen-select.php?<?=SID ?>"> 
