...
while ( is_array($_REQUEST)
  ...
  case "back":
            ...
            header("Location: http://$HTTP_HOST".$url);  // switches to page 'seitel.php'
            exit;
...
?>
<br>
  <form action="<?php $h="repgen-seiteline.php?".SID; echo $h; ?>" method="POST">
    <table border = 0>
      <TR>
        <TD>
          <B><?php echo IT_ART.":"; ?></B>
	  ...
	  <BR>
        ...
        <TD>
         ...
         <input type="hidden" name="id_neu" value="<?php echo $id_neu; ?>" >
          <input type="hidden" name= "sel_typ" value = "Line" >
...
?>
 <!-- existing items -->
 <form name="edit" method="post" action="<?php echo "repgen-seiteline.php?".SID; ?>">
...
<?php if (in_array($it_typ,array("Line"))) { ?>
    <TD>
