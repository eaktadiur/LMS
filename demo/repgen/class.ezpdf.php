...
function ezColumnsStart($options=array()){
  ...
  // setup the columns
  if (!isset($this->ez['columns'])){
    $this->ez['columns']=array('on'=>1,'colNum'=>1);
  } else {
    $this->ez['columns']['on']=1;
  }
  ...
  foreach($data as $row){
    ...
    while(!$abortTable && ($newPage || $newRow)){
      ...
      if ($newPage || $y<$this->ez['bottomMargin'] || (isset($options['minRowSpace']) && $y<($this->ez['bottomMargin']+$options['minRowSpace'])) ){
        ...
        $this->ezNewPage();
        // and the margins may have changed
        ...
        $mx=0;
        if ($options['showHeadings']){
          $mx = $this->ezPrvtTableColumnHeadings($cols,$pos,$maxWidth,$height,$decender,$options['rowGap'],$options['fontSize'],$y,$options);
          $y1=$y;
        } else {
          $y1=$y0;
        }
...
//        $this->reopenObject($textObjectId);
    ...
    if ($options['splitRows']==0){
      if ($this->ezPageCount != $pageStart && $secondTurn==0){
