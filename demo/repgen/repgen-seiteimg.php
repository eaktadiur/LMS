...
global $x1, $y1, $width, $height, $datei, $long, $report_type, $bgcolor;
...
if (array_key_exists('height', $_REQUEST)){$height = $_REQUEST["height"];};
if (array_key_exists('datei', $_REQUEST)){$datei = $_REQUEST["datei"];};
...
while ( is_array($_REQUEST)
  ...
  case "insert":
               //  inserts item into table reports
                      // test the input 
              // test if jpeg or npg datei  
         $h_a = split("\.", $datei); 
         $h_a_last = trim($h_a[count($h_a)-1]);   // split file type  $h_a_last should be filetype 
         if (!in_array($h_a_last,array("jpg","png"))) {
            $error = ERROR_FILENAME;
	    }
            else {
            if (!file_exists($datei)) {
            $error = ERROR_FILE_EXISTS.$datei."!";
            } else {
            $attrib1 = $sel_typ."|".$sel_art."|".$datei."|".$x1."|".$y1."|".$width."|".$height."|||||||||";
            if (!(empty($id_neu) or empty($x1) or empty($y1) or empty($width) or empty($height) or empty($datei)))  {
...
<H3> <?php echo ITEM_IMG; ?> </H3>
<?php if (!empty($error))
    my_error($error);
?>
<br>
  <form action="<?php $h="repgen-seiteimg.php?".SID; echo $h; ?>" method="POST">
    <table border = 0>
      <TR>
        <TD>
          <B><?php echo IT_ART.":"; ?></B>
...
<?php   }  ?>
        ...
        <TD>
          ...
          <input type="text" name="height" size="4" maxlength="4" value="<?php if (isset($height)) { echo $height; } ?>" onBlur="num_test(this);">
        </td>
    </TR>
</table>
<br>
          <B><?php echo IT_FILE;?>:</B>
          <input type="text" name="datei" size="50" maxlength="50"  value="<?php echo $datei;?>">

<br><BR>
         <input type="hidden" name="id_neu" value="<?php echo $id_neu; ?>" >
          <input type="hidden" name= "sel_typ" value = "Img" >
...
<table border=0 bgcolor="#eeeeee" align="center" cellspacing=2 cellpadding=2 width=540>
 <tr valign=top align=left bgcolor = <?php echo BGCOLORH; ?>>
  ...
  <th><?php echo IT_HEIGHT; ?></th>
  <th><?php echo IT_LINE; ?></th>
...
?>
 <!-- existing items -->
 <form name="edit" method="post" action="<?php echo "repgen-seiteimg.php?".SID; ?>">
...
<?php if (in_array($it_typ,array(IMAGE))) { ?>
    <TD>
