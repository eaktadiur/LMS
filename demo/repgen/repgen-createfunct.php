 ...
 * repgen-createfunction.php for PHP Report Generator
   Bauer, 5.2.2002
   Version 0.2
*/
...
if (array_key_exists('short', $_REQUEST)){$short = $_REQUEST["short"];}
if (array_key_exists('long', $_REQUEST)){$long = $_REQUEST["long"];}
if (array_key_exists('author', $_REQUEST)){$author = $_REQUEST["author"];}
if (array_key_exists('function', $_REQUEST)){
               $function = stripslashes($_REQUEST["function"]);
               }
...
if (empty($function) and !empty($short)) {
...
<br>
 ...
 <form name="edit" method="post" action="<?php echo "repgen-createfunct.php?".SID; ?>" >
 <table>
    ...
    <TR><TD align = right><?php echo FUNKTION.":"; ?> </TD><TD><textarea name="function" rows=10 cols=50 wrap=virtual><?php if (empty($function)) echo ''; else echo $function; ?></textarea></td></TR>
